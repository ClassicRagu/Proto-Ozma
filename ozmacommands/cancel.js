...
} = require('../functions/EmbedFunctions/buildExternalAnnouncementEmbeds')
    ...
    if (args.length === 1) {
      pool.query('UPDATE `Runs` SET `Cancelled` =  1 WHERE `ID` = ?', [
        ...
        pool
          ...
          .then((row) => {
            let announceCancel = buildExternalAnnounceCancelled(
              row,
              cafe,
              `This ${config.serverAbbr} run has been canceled.`
            )
            ...
            channelLeads.messages
              ...
              .catch(() => {
                console.error(
                  'Party Lead message failed to delete, run had likely already started.'
                )
              })
