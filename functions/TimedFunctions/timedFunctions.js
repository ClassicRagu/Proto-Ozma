const {
  getCountdownString
} = require('../CountdownFunctions/getCountdownString')
const {
  getFineCountdownString
} = require('../CountdownFunctions/getFineCountdownString')
const {
  getDate,
  getDayOfWeek,
  getServerTime
    ...
    pool
      ...
      .then((row) => {
        ...
        row.forEach((run) => {
          ...
          if (run.Plusone || Math.round(run.Start) < Date.now() + 7200000) {
          ...
          if (run.Type === 'Normal') {
            embedDescription += `**${run.ID}: ${
              ...
              run.ID
            } Party Leader Sign-up](https://discord.com/channels/750103971187654736/958076900880830545/${
              run.EmbedID
            })\n\n`
          } else {
            embedDescription += `**${run.ID}: ${
          ...
          .then((msg) => {
            if (!msg.embeds[0].description.includes(embedHash)) {
              embedDescription +=
                ...
                embedHash +
                '||'
              let embedSchedule = buildScheduleEmbed(
                serverInfo,
                embedDescription
              )
    ...
    pool
      ...
      .then((row) => {
        if (typeof row[0] !== 'undefined') {
          ...
          let embedCountdown = buildCountdownEmbed(
            serverInfo,
            cafe,
            row,
            targetDate
          ...
          channelServerTime.messages
            ...
            .then((msg) => {
              if (
                msg.embeds[0].title != embedCountdown.title ||
                msg.embeds.fields === undefined ||
          ...
          if (timeString === '55min.' && row[0].PasscodeMain > 0) {
            ...
            passcodeChannel.setName(
              serverInfo.emoji.passcodeLock +
                'Password in ' +
                getCountdownString(targetDate) +
                serverInfo.emoji.passcodeLock
            )
            client.users.cache
              .get(row[0].RL)
          ...
          if (timeString === '30min.') {
            ...
            if (row[0].PasscodeMain > 0) {
              ...
              client.channels.cache
                .get(serverInfo.channels.passcodePG)
                .send({ embeds: [embedNoPasscode] })
            }
          }
          if (timeString === '15min.') {
            channelArsenal.send(
          ...
          if (timeString === '1min.' || row[0].Cancelled === true) {
            let announceEdit = buildExternalAnnounceOngoing(
              row,
              cafe,
              `This ${config.serverAbbr} run has started.`
            )
            channelLeads.messages
              .fetch(row[0].EmbedID)
              .then((message) => {
                message.delete()
              })
          ...
          if (
            Math.round(row[0].Start) < Date.now() + 86400000 &&
            row[0].AnnounceEmbedID === null &&
            row[0].Cancelled === 0 &&
            !exBlacklist.includes(blistlead.toString()) &&
            !row[0].noAnnounce
          ) {
            let runID = row[0].ID
