...
const { buildDRSEmbed, buildExternalAnnounceNewDRS, } = require('../EmbedFunctions/index')
      ...
      pool.query(
        ...
        .then((row) => {
          ...
          row.forEach((run) => {
            ...
            if (run.DRS) {
                embedDescription += `\n**${run.ID}: ${run.Type} Run**\n●Your *Local* Start Time: __<t:${Math.round(run.Start / 1000)}:F>__\n ● Organizer: ${raidLeader}\n● Recruitment/Sign-up: [${run.Description}](https://discord.com/channels/750103971187654736/1167469922830536704/${run.EmbedID})`;
            }
          });
          if (embedDescription === "") {
            embedDescription =
