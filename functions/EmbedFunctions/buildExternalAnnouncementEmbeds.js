...
const generateStrikethroughFieldsDRS = (row, cafe) => {
  return [{ name: "~~Where:~~", value: `~~[${config.serverAbbr} on ${config.datacenter} DC](${config.discordLink})~~`, inline: false },
  { name: "~~Raid Leader~~", value: `~~${cafe.members.cache.get(row[0].RL).displayName}~~`, inline: true },
  { name: "~~Type~~", value: `~~${row[0].Type}~~`, inline: true },
  { name: "~~Your Local Start Time~~", value: `~~<t:${Math.round(row[0].Start / 1000)}:F>~~` },
  { name: "~~Time Till Run~~", value: `~~<t:${Math.round(row[0].Start / 1000)}:R>. Roster usually posted within 24hrs of start time.~~` },
  { name: "~~Run Title~~", value: `~~${row[0].Description}~~` },
  { name: "~~Run ID~~", value: `~~${row[0].ID.toString()}~~` }]
}
...
const buildExternalAnnounceOngoingDRS = (row, cafe, title) => {
  return new EmbedBuilder()
    .setColor(serverInfo.announceEmbedColor)
    .setTitle(title)
    .setThumbnail(`${serverInfo.announceGifs.announceStartedDRS}`)
    .addFields(generateStrikethroughFieldsDRS(row,cafe))
}
...
const buildExternalAnnounceNewDRS = (row, cafe) => {
  return new EmbedBuilder()
    .setColor(serverInfo.announceEmbedColor)
    .setTitle(`New DRS Opportunity On ${config.serverAbbr}`)
    .setThumbnail(`${serverInfo.announceGifs.announceNewDRS}`)
    .addFields({ name: "Where:", value: `[${config.serverAbbr} on ${config.datacenter} DC](${config.discordLink})`, inline: false },
      { name: "Raid Leader", value: cafe.members.cache.get(row[0].RL).displayName, inline: true },
      { name: "Type", value: `${row[0].Type}`, inline: true },
      { name: "Your Local Start Time", value: `<t:${Math.round(row[0].Start / 1000)}:F>` },
...
module.exports = { buildExternalAnnounceOngoing, buildExternalAnnounceCancelled, buildExternalAnnounceNewRun, buildExternalAnnounceNewDRS, buildExternalAnnounceOngoingDRS }
